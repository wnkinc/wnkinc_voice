...
3. **Seed**: `TENANTS_TABLE=<tenantsTableName output> PEOPLE_TABLE=<peopleTableName output> COGNITO_USER_POOL_ID=<userPoolId output> COGNITO_RESOURCE_SERVER_ID=<resourceServerId output> AWS_REGION=us-west-2 npm run seed -- tenants/<id>.json`. If the file has no `cognitoClientId`, the seed mints the tenant's Gateway identity (a Cognito app client) and writes the id back into the file — commit it. Every agent acting for the tenant calls the Gateway as that client; the Gateway's interceptor maps it back to the tenant and injects tenant context into every tool call. No policy or gateway change per tenant: the Gateway validates by scope.
...
5c. **HubSpot consent** (if `crm` is set): `npx tsx scripts/connect-composio.mts <id> hubspot` — the owner approves Composio's HubSpot app once; the token lives in Composio's vault under the tenant id. Set `crm: { "type": "hubspot", "via": "composio" }`. Prove with `npx tsx scripts/test-crm.mts <id> <a known phone>`.
5d. **LinkedIn consent** (if `products.linkedin.enabled`): `npx tsx scripts/connect-composio.mts <id> linkedin` — the owner approves Composio's LinkedIn app once; the assistant then posts as them. No LinkedIn developer app needed (Composio-managed OAuth).
