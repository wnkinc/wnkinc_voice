...
async function tabBusiness(){
  ...
  const groups=[
    ...
    ['Integrations',[['owner alerts (Telegram)',(c.people||[]).filter(p=>p.role==='owner'&&p.telegramId).map(p=>p.name).join(', ')||'(no owner with a telegramId — notify_owner alerts fail)'],['crm',c.crm?.type||'(none)'],['products (services on)',Object.entries(c.products||{}).filter(([,v])=>v.enabled).map(([k,v])=>k+(v.via?' via '+v.via:'')).join(', ')||'(none — voice only)']]],
  ];
