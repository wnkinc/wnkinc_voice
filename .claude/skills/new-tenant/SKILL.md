...
9. **Assistant tools** (if `assistant.enabled`): list the tools in `assistant.tools`, by name from the catalog in `packages/worker/src/rules/assistant.ts` (`search_contacts`, `add_note`; both need the HubSpot consent above), and any Composio toolkit the assistant reaches as Composio's own tools under `assistant.composioTools` with its tool slugs (`"googlecalendar": ["GOOGLECALENDAR_FIND_EVENT", ...]`, after `npx tsx scripts/connect-composio.mts <id> googlecalendar`). Nothing is minted and nothing deploys: the seed writes the row and the loop reads it.
