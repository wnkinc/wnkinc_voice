...
/** An MCP session for this turn over the toolkits and tools the row lists (assistant.mcp); the model activity calls it with the key. */
export const mcpSession = (tenantId: string, tools: Record<string, string[]>, timezone?: string): Promise<string> => composio.mcpSession(tenantId, tools, timezone);
