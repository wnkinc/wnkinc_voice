...
6c. **Facebook posts** (if they want to text photos and have the assistant post them to their Page): they need a Facebook Page and admin access to it (not a personal profile). `npx tsx scripts/connect-composio.mts <id> facebook`, ticking the Page on the consent screen; then in the tenant file `"facebookPosts": { "enabled": true, "pageId": "<numeric id>", "pageName": "<name>" }` (both from `FACEBOOK_GET_USER_PAGES` for that tenant) and `draft_facebook_post`, `cancel_facebook_draft` in `assistant.tools`; re-seed. SMS only. Nothing deploys. The person's reply POST publishes, never the model (`workflows/assistant/facebook-post.ts`).
