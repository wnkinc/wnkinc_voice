...
9. **Assistant tools** (if `assistant.enabled`): list the tools in `assistant.tools`, by name from the catalog in `packages/worker/src/rules/assistant.ts` (`search_contacts`, `add_note`; both need the HubSpot consent above). Nothing is minted and nothing deploys: the seed writes the row and the loop reads it.
