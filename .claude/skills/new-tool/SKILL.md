...
2. **Credentials** in `lib/identity-stack.ts`:
   ...
   - OAuth (3LO): `OAuth2CredentialProvider.using<Vendor>(...)`; output its `callbackUrl` and register it in the vendor's OAuth app; run a consent flow like `scripts/connect-google.ts`.
