---
name: new-agent
description: Add a new agent (a new surface/system) — a workflow on the worker, or a Lambda on an event, with its package, identity, and wiring. Use when adding a channel, a scheduler, a dashboard API, or any new autonomous behavior.
---
...
2. **Shared code**: anything another deployable also needs goes in `packages/shared` (imported as `@wnk/shared`, or a subpath such as `@wnk/shared/contracts`). Deployables never import each other's source; the shared package is the one allowed edge, and its contracts are what the stacks, the tenant files and the worker agree on. The worker's image installs the shared package and its dependencies, so nothing heavy goes there.
3. **Hosting**: a workflow needs none (it is in the worker's image; grant the worker what its activities touch in `stacks/worker-stack.ts` `grantWorker`). A Lambda goes in the stack of the system it serves (`receptionist-stack.ts` for the call path): a `NodejsFunction` (ESM, node22, ARM, X-Ray active, a dead-letter queue, `dlqAlarm` + `errorAlarm`) with env vars for everything it needs. If it imports a CommonJS package, add the `createRequire` banner the receptionist-stack `fn` helper uses.
4. **Identity**: the agent acts for the tenant selected upstream — an automation takes `tenantId` from the event and passes it to every Composio adapter call; the assistant loop runs each tool through Composio naming the tenant from the row. No agent holds a credential.
5. **Grants**: the execution role gets exactly what the agent touches — tables, secrets, memory actions (`MEMORY_USE_ACTIONS` on the memory ARN + `/*`). Expect to discover one missing action from an AccessDenied message; the error names the exact action + resource — encode it, don't wildcard the service.
...
Lambdas: `/aws/lambda/<function name>`. Workflows: the history (`npm run temporal -- workflow show --workflow-id <id>`: every activity with its input and result) and the worker's log group, `/aws/lambda/wnk-dev-worker`, where the failure alarms read from.
