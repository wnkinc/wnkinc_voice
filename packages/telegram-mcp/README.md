...
| Secrets Manager | The tenant-named secret `wnkinc-voice-dev/<id>/telegram-mcp`: `TELEGRAM_API_ID`, `TELEGRAM_API_HASH`, `TELEGRAM_SESSION_STRING`, and `URL_TOKEN` (generated by the stack, never typed by a person). ~$0.40/mo. | |
...
4. `npx cdk deploy wnk-telegram-mcp-<id>-dev`. It creates the secret with a generated `URL_TOKEN` and the three Telegram values empty; the function refuses to start until they are filled.
5. Fill them, entered at hidden prompts so nothing lands in shell history or chat:
   ```
   T=<tenantId>; S=wnkinc-voice-dev/$T/telegram-mcp
   ...
   ```
   Then force a fresh instance so it reads them: `aws lambda update-function-configuration --region us-west-2 --function-name wnkinc-voice-dev-$T-telegram-mcp --description "credentials $(date +%F)"`.
...
```
A wrong token is a 404. Logs: `/aws/lambda/wnkinc-voice-dev-<id>-telegram-mcp`; an unfilled secret key or a revoked session fails the init there, naming which.
...
- **Offboard**: the tenant ends the session in Telegram → Settings → Devices (this alone cuts access), then remove `telegramMcp` from their file and `npx cdk destroy wnk-telegram-mcp-<id>-dev`. The secret goes to a 30-day recovery window; `aws secretsmanager delete-secret --secret-id wnkinc-voice-dev/<id>/telegram-mcp --force-delete-without-recovery` ends it now, and is required before redeploying the same tenant.
