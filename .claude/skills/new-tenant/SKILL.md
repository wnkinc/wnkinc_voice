...
7c. **Telegram connector** (if the tenant wants their Telegram account in their ChatGPT or Claude): `telegramMcp: true` in `tenants/<id>.ts`, their secrets under `/wnkinc-voice-dev/<id>/telegram-mcp/`, then `npx cdk deploy wnk-telegram-mcp-<id>-dev`. The steps, with the QR login and the hidden-prompt secret entry, are in `packages/telegram-mcp/README.md`. A tenant who wants only the connector skips everything else here: no number, no row, no seed, `automations: []` (like `tenants/meg.ts`).
