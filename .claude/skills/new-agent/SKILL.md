...
3. **Hosting**: a workflow needs none (it is in the worker's image; grant the worker what its activities touch in `stacks/worker-stack.ts` `grantWorker`). A Lambda goes in the stack of the system it serves (`receptionist-stack.ts` for the call path): a `NodejsFunction` (ESM, node22, ARM, X-Ray active, a dead-letter queue, `dlqAlarm` + `errorAlarm`) with env vars for everything it needs. If it imports `@wnk/shared/composio`, add the `createRequire` banner the receptionist-stack `fn` helper uses.
